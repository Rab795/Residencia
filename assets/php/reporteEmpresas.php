<?php
	...
	$campos="DISTINCT emp_id,emp_nombre,emp_RFC,emp_direccion,emp_ramo,emp_tel";
	if($proyecto == ""){
		$tables="Empresa LEFT JOIN Proyecto ON pro_idEmpresa = emp_id";
		$sWhere=" emp_nombre LIKE '%".$nombre."%'";
		$sWhere.=" AND emp_RFC LIKE '%".$rfc."%'";
		$sWhere.=" AND emp_ramo LIKE '%".$ramo."%'";
	}else if($proyecto == 1){
		$tables="Empresa INNER JOIN Proyecto ON pro_idEmpresa = emp_id";
		$sWhere=" emp_nombre LIKE '%".$nombre."%'";
		$sWhere.=" AND emp_RFC LIKE '%".$rfc."%'";
		$sWhere.=" AND emp_ramo LIKE '%".$ramo."%'";
	}else{
		$tables="Empresa LEFT JOIN Proyecto ON pro_idEmpresa = emp_id";
		$sWhere=" pro_idEmpresa IS NULL AND emp_nombre LIKE '%".$nombre."%'";
		$sWhere.=" AND emp_RFC LIKE '%".$rfc."%'";
		$sWhere.=" AND emp_ramo LIKE '%".$ramo."%'";
	}
	...
	$count_query   = mysqli_query($conectado,"SELECT count(*) AS numrows FROM (SELECT $campos FROM $tables where $sWhere) AS d ");
